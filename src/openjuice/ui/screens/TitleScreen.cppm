...
export module openjuice.ui.screens:TitleScreen;
 ...
 * @extends Screen
 */
export class TitleScreen final: public Screen {
private:
    ...
    void createComponent() noexcept override final {
        ...
        Component componentWithEvents = CatchEvent(menu, [this](Event event) -> bool {
            if ((event.is_mouse() && event.mouse().button == Mouse::Left) || (event == Event::Return)) {
                switch (selectedOption) {
                    case 0: // New Game
                        if (Expected<void, ProfileManager::Error> result = profile->resetProfile(); !result.has_value()) {
                        ...
                        return true;
                    case 1: // Continue
                        if (Expected<void, ProfileManager::Error> result = profile->loadProfile(); !result.has_value()) {
        ...
        component = Renderer(componentWithEvents, [this] -> Element {
            return vbox({
                ...
                menu->Render() | center,
                separator(),
                text(Ops::fmt("Version 0.0.1\nCompiled with {} for {}", Environment::compiler(), Environment::operating_system())) | center,
            }) | border | flex;
...
public:
     ...
     */
    TitleScreen(SharedPointer<Game> game, Host& host, SharedPointer<LocalizationService> localization, SharedPointer<ProfileManager> profile):
