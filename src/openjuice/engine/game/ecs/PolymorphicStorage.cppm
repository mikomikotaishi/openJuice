...
public:
    ...
    PolymorphicStorage(const PolymorphicStorage& other):
        ...
        connector{other.populated ? Pointers::unique<EntityId[]>(other.capacity) : nullptr},
        indices{other.populated ? Pointers::unique<u32[]>(other.capacity) : nullptr},
    ...
    PolymorphicStorage& operator=(const PolymorphicStorage& other) noexcept {
        if (this != &other) {
            ...
            if (other.populated) {
                ...
                connector = Pointers::unique<EntityId[]>(capacity);
                stdx::ranges::copy(Span<EntityId>(other.connector.get(), capacity), connector.get());
    ...
    PolymorphicStorage* populate(u32 cap) noexcept {
        ...
        mask = Pointers::unique<bool[]>(cap);
        connector = Pointers::unique<EntityId[]>(cap);
        indices = Pointers::unique<u32[]>(cap);
